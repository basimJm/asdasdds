...
public class ParentProfile extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        RequestQueue queue = VolleySingleton.getInstance(ParentProfile.this).getRequestQueue();
        final String url = AppConstants.SERVER_URLS.SERVER_URL + AppConstants.SERVER_URLS.USER_PROFILE_URL+"?u_email_id="+userEmailId+"&u_id="+userId+"&token="+token+"&role_id="+roleId+"&device_id="+Preferences.getInstance().deviceId+"stu_id="+Preferences.getInstance().studentId;
            ...
            public void onResponse(String response) {
                ...
                try
                {
                    ...
                    if(responseObject.has("Msg")&&responseObject.getString("Msg").equals("0"))
                        Utils.showToast(ParentProfile.this,"No Records Found");
                    else
                    if(responseObject.has("error")&&responseObject.getString("error").equals("0"))
                    {
                        Utils.showToast(ParentProfile.this, "Session Expired:Please Login Again");
                    }
                    else
                    if(responseObject.has("profile"))
                    {
                        ...
                        if(null!=userProfileArray && userProfileArray.length()>=0)
                        {
                            ...
                            Glide.with(ParentProfile.this).load(AppConstants.SERVER_URLS.IMAGE_URL+responseObject.getJSONArray("profile").getJSONObject(0).getString("picture")).placeholder(R.drawable.profilebig).error(R.drawable.profilebig).into(studentImage);
                            Glide.with(ParentProfile.this).load(AppConstants.SERVER_URLS.IMAGE_URL+responseObject.getJSONArray("profile").getJSONObject(0).getString("father_pic")).placeholder(R.drawable.profilebig).error(R.drawable.profilebig).into(studentFatherImage);
                            Glide.with(ParentProfile.this).load(AppConstants.SERVER_URLS.IMAGE_URL+responseObject.getJSONArray("profile").getJSONObject(0).getString("mother_pic")).placeholder(R.drawable.profilebig).error(R.drawable.profilebig).into(studentMotherImage);
                        ...
                        Utils.showToast(ParentProfile.this, "Error Fetching Response");
                ...
                {
                    e.printStackTrace();
                    Utils.showToast(ParentProfile.this, "Error fetching modules! Please try after sometime.");
                    setSupportProgressBarIndeterminateVisibility(false);
                }
        ...
        if(Utils.isNetworkAvailable(ParentProfile.this))
            queue.add(requestObject);
        else
        {
            Utils.showToast(ParentProfile.this, "Unable to fetch data, kindly enable internet settings!");
        }
