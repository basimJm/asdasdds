...
import com.schoofi.adapters.IncomingVoiceMessageViewHolder;
import com.schoofi.adapters.OutcomingVoiceMessageViewHolder;
...
public class StudentChatScreen extends AppCompatActivity implements MessageInput.InputListener{
    ...
    protected void getStudentFeedList()
    {
        ...
        RequestQueue queue = VolleySingleton.getInstance(StudentChatScreen.this).getRequestQueue();
        final String url = AppConstants.SERVER_URLS.SERVER_URL + AppConstants.SERVER_URLS.STUDENT_DIARY_CHAT_SERVICE+"?token="+Preferences.getInstance().token+"&device_id="+Preferences.getInstance().deviceId+"&from_user_id="+Preferences.getInstance().userId+"&recipient="+recipient_id+"&type="+type;
                ...
                try
                {
                    ...
                    if(responseObject.has("responseObject"))
                    {
                        ...
                        if(null!=AssignmentArray && AssignmentArray.length()>=0)
                        {
                            ...
                            VolleySingleton.getInstance(StudentChatScreen.this).getRequestQueue().getCache().put(AppConstants.SERVER_URLS.SERVER_URL + AppConstants.SERVER_URLS.STUDENT_DIARY_CHAT_SERVICE+"?token="+Preferences.getInstance().token+"&device_id="+Preferences.getInstance().deviceId+"&from_user_id="+Preferences.getInstance().userId+"&recipient="+recipient_id+"&type="+type,e);
                            ...
                            assignmentMultiLevelListAdapter = new ChatMainAdapter(jsonArray,StudentChatScreen.this,diaryVOs);
                            expandableListView.setLayoutManager(new LinearLayoutManager(StudentChatScreen.this, RecyclerView.VERTICAL, false));
            ...
            public void onResponse(String response) {
                ...
                try
                {
                    ...
                    if(responseObject.has("Msg")&&responseObject.getString("Msg").equals("1"))
                    {
                        ...
                        Intent mIntent = new Intent(StudentChatScreen.this,StudentChatScreen.class);
